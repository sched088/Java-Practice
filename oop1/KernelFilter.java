...
// I've tried for three weeks' worth of lectures to get this going...
...
public class KernelFilter {
    ...
    public static Picture gaussian(Picture picture) {
        ...
        for (int col = 1; col < (patch.width() - 1);
             col++) {  //center elements copied from original picture
        ...
        for (int col = 0; col < patch.width();
             col += (patch.width() - 1)) { //left & right patch elements
            for (int row = 1; row < (patch.height() - 1); row++) {
                if (col == 0) {
        ...
        for (int row = 0; row < patch.height();
             row += (patch.height() - 1)) { //up & down 2 patch elements
        ...
        for (int row = 1; row < patch.height() - 1;
             row++) {                      //Multiplying Kernel
    ...
    public static Picture motionBlur(Picture picture) {
        ...
        for (int col = 4; col < (patch.width() - 4);
             col++) {  //center elements copied from original picture
        ...
        for (int col = patch.width() - 4; col < patch.width();
             col++) {          //right patch w-4 ~ w-1
            for (int row = 4; row < (patch.height() - 4); row++) {
        ...
        for (int row = patch.height() - 4; row < patch.height();
             row++) {       //down patch h-4 ~ h-1                                 //down patch h-4 ~ h-1 row
            for (int col = 0; col < patch.width(); col++) {
        ...
        for (int row = 4; row < patch.height() - 4;
             row++) {                      //Multiplying Kernel
